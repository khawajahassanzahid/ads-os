...
async function loadBrands() {
  try {
    const r = await store.list("brand:");
    if (!r?.keys?.length) return [];
    const items = await Promise.all(r.keys.map(async k => { try { const d = await store.get(k); return d ? JSON.parse(d.value) : null; } catch { return null; } }));
    return items.filter(Boolean).sort((a, b) => a.createdAt - b.createdAt);
  } catch { return []; }
}
async function saveBrand(b) { await store.set(`brand:${b.id}`, JSON.stringify(b)); }
async function loadChats(bid) {
  try {
    const r = await store.list(`chat:${bid}:`);
    if (!r?.keys?.length) return [];
    const items = await Promise.all(r.keys.map(async k => { try { const d = await store.get(k); return d ? JSON.parse(d.value) : null; } catch { return null; } }));
    return items.filter(Boolean).sort((a, b) => b.updatedAt - a.updatedAt);
  } catch { return []; }
}
async function saveChat(bid, chat) { await store.set(`chat:${bid}:${chat.id}`, JSON.stringify(chat)); }
async function deleteChat(bid, cid) { try { await store.delete(`chat:${bid}:${cid}`); } catch {} }
async function loadBlueprints(bid) {
  try {
    const r = await store.list(`bp:${bid}:`);
    if (!r?.keys?.length) return [];
    const items = await Promise.all(r.keys.map(async k => { try { const d = await store.get(k); return d ? JSON.parse(d.value) : null; } catch { return null; } }));
    return items.filter(Boolean).sort((a, b) => b.createdAt - a.createdAt);
  } catch { return []; }
}
async function saveBlueprint(bid, bp) { await store.set(`bp:${bid}:${bp.id}`, JSON.stringify(bp)); }
...
- Monthly budget: ${brand?.monthlyBudget ? "$" + Number(brand.monthlyBudget).toLocaleString() : "Not set"}
...
- Monthly budget: $${brand.monthlyBudget || 5000}
...
Brand: ${brand.name}, Industry: ${brand.industry || "General"}, Budget: $${brand.monthlyBudget || "unknown"}/mo
...
export default function AdsOS() {
  ...
  const [brandForm, setBrandForm] = useState({ name:"",industry:"",website:"",monthlyBudget:"",goals:"",metaAccountId:"",googleAccountId:"",notes:"",color:"#0082FB" });
  ...
  const saveBrandForm = async () => {
    ...
    setBrandForm({ name:"",industry:"",website:"",monthlyBudget:"",goals:"",metaAccountId:"",googleAccountId:"",notes:"",color:"#0082FB" });
  ...
  return (
    <div style={S.app}>
      ...
      <div style={S.topbar}>
        ...
        {!activeBrand && <button style={S.btn("#0082FB")} className="hov" onClick={() => { setEditingBrand(null); setBrandForm({name:"",industry:"",website:"",monthlyBudget:"",goals:"",metaAccountId:"",googleAccountId:"",notes:"",color:"#0082FB"}); setShowBrandModal(true); }}>+ Brand</button>}
      </div>
      ...
      <div style={S.main}>
        ...
        <div style={S.sidebar}>
          <div style={{ padding:"14px 12px 8px", overflowY:"auto", flex:1, minWidth:248 }}>
            ...
            <div className="hov-bg" onClick={() => { setEditingBrand(null); setBrandForm({name:"",industry:"",website:"",monthlyBudget:"",goals:"",metaAccountId:"",googleAccountId:"",notes:"",color:"#0082FB"}); setShowBrandModal(true); }} style={{ display:"flex", alignItems:"center", gap:9, padding:"7px 9px", borderRadius:9, cursor:"pointer", marginTop:2 }}>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(190px,1fr))", gap:10 }}>
                    {brands.map(b => (
                      <div key={b.id} className="hov-card" onClick={() => openBrand(b)} style={{ ...S.card, cursor:"pointer", textAlign:"left", transition:"all 0.2s" }}>
                        ...
                        {b.monthlyBudget && <div style={{ fontSize:11, color:b.color||"#0082FB", marginTop:5 }}>${Number(b.monthlyBudget).toLocaleString()}/mo</div>}
                      </div>
                    ))}
                    <div className="hov-card" onClick={() => { setEditingBrand(null); setBrandForm({name:"",industry:"",website:"",monthlyBudget:"",goals:"",metaAccountId:"",googleAccountId:"",notes:"",color:"#0082FB"}); setShowBrandModal(true); }} style={{ ...S.card, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", color:"#2A3550", fontSize:13, transition:"all 0.2s", border:"1px dashed #1E2535", background:"transparent" }}>+ Add Brand</div>
                  </div>
                )}
              </div>
            </div>
                ...
                <div style={{ flex:1, overflowY:"auto", padding:"24px 24px" }}>
                  <div style={{ maxWidth:800, margin:"0 auto" }} className="fade-up">
                    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:24 }}>
                      ...
                      <button style={S.btn(bc, true)} className="hov" onClick={() => { setEditingBrand(activeBrand); setBrandForm({name:activeBrand.name, industry:activeBrand.industry||"", website:activeBrand.website||"", monthlyBudget:activeBrand.monthlyBudget||"", goals:activeBrand.goals||"", metaAccountId:activeBrand.metaAccountId||"", googleAccountId:activeBrand.googleAccountId||"", notes:activeBrand.notes||"", color:activeBrand.color||"#0082FB"}); setShowBrandModal(true); }}>Edit</button>
                    </div>

                    {/* Account status cards */}
                    <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:24 }}>
                      {[
                        { label:"Budget / mo", value: activeBrand.monthlyBudget ? `$${Number(activeBrand.monthlyBudget).toLocaleString()}` : "Not set", color:bc },
          ...
          <div style={{ background:"#08090F", border:"1px solid #1E2535", borderRadius:20, padding:24, width:"100%", maxWidth:540, maxHeight:"90vh", overflowY:"auto" }} className="fade-up">
            ...
            <div style={{ display:"grid", gap:14 }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                ...
                <div><label style={{ fontSize:10, color:"#2A3550", letterSpacing:"0.06em", textTransform:"uppercase", display:"block", marginBottom:5 }}>Monthly Budget ($)</label><input style={S.input} type="number" value={brandForm.monthlyBudget} onChange={e => setBrandForm(p=>({...p,monthlyBudget:e.target.value}))} placeholder="10000" /></div>
              </div>
